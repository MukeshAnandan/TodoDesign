...
export class SubtaskComponent {
  ...
  /**
   * It is used to get the close the subtask panel
   */
  closeSidepanel() {
  ...
  /**
   * It is used to delete the particular task
   * @param task -used to delete that task 
   */
  deleteTask(task) {
    console.log(this.list.tasks);
    this.list.tasks.splice(this.list.tasks.indexOf(task), 1)
    this.list.taskCount = this.list.tasks.length;
    this.closeSidepanel();
  }
  
  /**
   * It is used to delete the particular subtask
   * @param subtask -used to delete the subtask
   */
  deleteSubTask(subtask){
    this.task.subtasks.splice(this.task.subtasks.indexOf(subtask),1)
    this.task.subtaskcount = this.task.subtasks.length;
    if(subtask.isFinished === true) {
      this.task.completedSubtaskcount -= 1;
    }
  } 
}
